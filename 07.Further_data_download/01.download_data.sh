#!/bin/bash

# Define the list of additional SRA data to download
# SRR11300652 - Australia
# SRR11397727 - Australia
# SRR11393278 - USA
# SRR11393277 - USA
# SRR11347377 - USA
# SRR11313287 - Wuhan
# SRR11313494 - Wuhan
# SRR10902284 - SZ
# SRR10948474 - SZ
# SRR11178057 - HK (already downloaded)
# SRR11178056 - HK (already downloaded)

additional_nanopore_sra_list=(SRR11300652 SRR11397727 SRR11393278 SRR11393277 SRR11347377 SRR11313287 SRR11313494 SRR10902284 SRR10948474 SRR11178057 SRR11178056)

for sra in ${additional_nanopore_sra_list[@]}
do
	if [ -f "../data/${sra}_pass.fastq.gz" ]; then
		echo "# $sra already downloaded!"
	else
		echo "# Downloading $sra..."
		fastq-dump --outdir ../data/ --gzip --skip-technical --readids --read-filter pass --dumpbase --clip $sra
	fi
done

